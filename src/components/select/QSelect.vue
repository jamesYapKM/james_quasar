      ...
      >
        <template v-if="multiple">
          <q-item-wrapper
            slot-replace
			:cfg="{label:'ALL', value:''}"
            @click.capture.native="__selectAll()"
          >
            <q-toggle
              v-if="toggle"
              slot="right"
...
export default {
  ...
  computed: {
    allModel () {
      if (this.multiple) {
        return JSON.stringify(this.allValue) === JSON.stringify(this.model)
      }
    },
    allValue () {
      const opt = this.options.map(opt => opt.value)
  ...
  methods: {
    ...
    __selectAll () {
      if (this.allValue.length === this.model.length) {
        this.model.splice(0, this.model.length)
        this.__emit([])
      }
      else {
        this.model = this.allValue
        this.__emit(this.allValue)
      }
    }
  }
}
